...
from os.path import exists, dirname, isdir, isfile, join, sep, splitext
...
class AbstractArchive():
    ...
    def inmemory(self, value):
        ...
        """
        self.tempdir = None if value else mkdtemp(prefix="pyrus.archives_")
    ...
    def check_unsafe(self):
        ...
        if not self.allow_unsafe_extraction:
            for filename in self.filelist:
                if filename.startswith(sep) or filename.startswith('..'):
                    raise Exception(
                        "Unsafe filename_from_info in archive %s"
                        % self.filepath)
...
class ZipFile(AbstractArchive):
    def __init__(self, filepath, fileobj=None, inmemory_processing=True,
        ...
        if fileobj:
            fileobj.seek(0)
        AbstractArchive.__init__(
            self, filepath, inmemory_processing, allow_unsafe_extraction)
    ...
    def extract_all(self, force_file_obj=False):
        ...
        if self.inmemory:
            ...
            for member in self.infolist():
                filepath = join(self.tempdir, member.filename)
...
class TarFile(AbstractArchive):
    ...
    def extract(self, member, force_file_obj=False):
        ...
        if self.inmemory:
            ...
            filepath = join(self.tempdir, filepath)
            return file_to_bio(filepath) if force_file_obj else filepath
...
class NativeTarFile(AbstractNativeArchive):
    def __init__(self, filepath, fileobj=None, inmemory_processing=False,
        ...
        self.path = filepath
        if fileobj:
            temp_path = join(mkdtemp(prefix='nativetar-'), filepath)
            makedirs(dirname(temp_path))
            with open(temp_path, 'wb') as f:
                f.write(fileobj.read())
                fileobj.seek(0)
            self.path = temp_path
        assert name in native_support_os
        AbstractArchive.__init__(
            self, filepath, inmemory_processing, allow_unsafe_extraction)
    ...
    def prepare_info(self):
        ...
        for line in output.decode().strip().split('\n'):
            ...
            if match:
                ...
                if filename.find('->') >= 0:
                    filename, linkto = [
                        i.strip() for i in filename.split('->')
                    ]
                permissions = match.group(1)
                owner, group = match.group(2).split('/')
                filesize = int(match.group(3))
                self.__infolist.append(NativeInfo(
                    filename, permissions, owner, group, filesize, linkto)
                )
...
def make_archive_obj(filepath, fileobj=None, inmemory_processing=True,
                     allow_unsafe_extraction=False):
    """
...
def is_tarfile(arg):
    ...
    if isinstance(arg, str):
        ...
        return splitext(arg.name)[-1] in ['tar', 'gz', 'bz', 'bz2']
    return False
